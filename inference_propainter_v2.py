...
def resize_frames(frames, size=None):    
    ...
    if size is not None:
        ...
        print('ProcessSize: ' + str(process_size))
    else:
...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5):
    print("READING FRAME WISE MASKS")
    masks_img = []
    masks_dilated = []
    flow_masks = []
...
def extrapolation(video_ori, scale):
    ...
    """
    print('EXTRAPOLATION for INPAINTING')
...
def generate_new_file_name(filename):
    flow_format_match = re.match(r"(.*_)(\d+)(\..*)", filename)
    if not flow_format_match:
            raise ValueError
        
    base, number, extension = flow_format_match.groups()
    original_number = int(number)
    new_filename = filename
    while os.path.exists(new_filename):
        original_number += 1
        new_filename = f"{base}{original_number}{extension}"
    
    return new_filename

def write_flow_viz(flow, filename):

    if flow.dim() == 2:
        # Assuming flow is (H, W), we need to add a dimension to represent two channels (assuming zero flow in the second channel)
        flow = flow.unsqueeze(0).repeat(2, 1, 1)  # Repeats the flow tensor to simulate two channels; adjust if your data differs.
    elif flow.dim() == 3 and flow.size(0) != 2:
        # If the first dimension is not 2, but there are three dimensions, assume it needs reordering or fixing.
        raise ValueError("The flow tensor has three dimensions but does not have 2 channels as the first dimension. Please check your data.")
    new_img_filename = generate_new_file_name(filename)
        
    print(flow.size())
    flow_image = flow_to_image(flow)
    pil_image = to_pil_image(flow_image)
    pil_image.save(new_img_filename)
...
def write_flow(flow, filename):
    ...
    with open(new_filename, 'wb') as f:
        np.array([80, 73, 69, 72], np.uint8).tofile(f) #Header
        np.array([flow.shape[1], flow.shape[0]], np.int32).tofile(f) #width and height
        np.array(flow, np.float32).tofile(f)

        


def unpack_tensor_batches(batch):
    original_tensor_f = batch[0][:, s_f:e_f]
    extracted_elements = []

    for i in range(original_tensor_f.size(1)):
        # Extracting the element and removing the singleton first dimension
        extracted_element = original_tensor_f[0, i, :, :, :]  # This removes the first dimension and selects the ith element of the second dimension
        extracted_elements.append(extracted_element)

    return extracted_element


def explore_flow(flow):
    print("")
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if frames.size(1) > short_clip_len:
            ...
            #print('SHORT CLIP LENGTH: ' + str(short_clip_len))
            #print('VIDEO LENGTH ' + str(video_length))
            
            for f in range(0, video_length, short_clip_len):
                #print('BEGINNING of F ' + str(f))
                end_f = min(video_length, f + short_clip_len)
                #print('END OF F ' + str(end_f))
                if f == 0:
                ...
                #print('SHAPE OF FLOWS - F = ' + str(flows_f.size()) + '  B ' + str(flows_b.size()))
                gt_flows_f_list.append(flows_f)
                gt_flows_b_list.append(flows_b)

                

                #print('CREATING FLOW BATCH ' + str(i) + ' TIMES')

                
                
                torch.cuda.empty_cache()
            #print('FORWARD FLOW LIST ' + str(gt_flows_f_list).size())
            #print('BACKWARDS FLOW LIST ' + str(gt_flows_b_list).size())
            #print('GETTING RAFT FLOWS')    
        ...
        if flow_length > args.subvideo_length:
            ...
            for f in range(0, flow_length, args.subvideo_length):
                ...
                #print('with FlowLength > subvideo length COMPLETING RAFT FLOW for frame ' + str(f))
                ...
                #TO DO: unpack the contents of pred_flows_bi_sub until you get to the object where the predicted flow of an individual frame is stored            
                ...
                original_tensor_f = pred_flows_bi_sub[0][:, pad_len_s:e_f-s_f-pad_len_e]
                extracted_elements = []

                print('dimensions of pred_flows_bi_sub[0] ' + str(len(pred_flows_bi_sub[0])))
                print('length of pred_flows_f ' + str(len(pred_flows_f)))

                for i in range(original_tensor_f.size(1)):
                    print('Extracting the element and removing the singleton first dimension')
                    extracted_element = original_tensor_f[0, i, :, :]  # This removes the first dimension and selects the ith element of the second dimension
                    extracted_elements.append(extracted_element)
                for flow in extracted_elements:
                    write_flow(flow.cpu(), f'flow_f_1.flo')
                    write_flow_viz(flow.cpu(), f'flow_viz_f_1.png')
                    write_flow(flow.cpu(), f'flow_b_1.flo')
                    write_flow_viz(flow.cpu(), f'flow_viz_b_1.png')

                original_tensor_f = pred_flows_bi_sub[1][:, pad_len_s:e_f-s_f-pad_len_e]
                extracted_elements = []
                ...
                print("length of extracted elements " + str(len(extracted_elements)))
        ...
        if video_length > subvideo_length_img_prop:
            ...
            for f in range(0, video_length, subvideo_length_img_prop):
                print("with video_length > subvideo IMAGE PROPAGATION FOR FRAME " + str(f))
                s_f = max(0, f - pad_len)
    ...
    for f in tqdm(range(0, video_length, neighbor_stride)):
        print("FEATURE PROPAGATION FOR FRAME " + str(f))
        neighbor_ids = [
